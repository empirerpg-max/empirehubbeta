...
function PopStarQuest() {
  ...
  const update = () => {
    const { player, platforms, enemies, items, bullets, keys, camera, levelWidth } = gameRef.current;
    if (!player) return;

    // Movement
    if (keys["ArrowLeft"] || keys["a"]) { player.vx = -WALK_SPEED; player.facing = -1; }
    else if (keys["ArrowRight"] || keys["d"]) { player.vx = WALK_SPEED; player.facing = 1; }
    ...
    for (const p of platforms) {
      if (rectIntersect(player.x, player.y, player.w, player.h, p.x, p.y, p.w, p.h)) {
         ...
         // Resolve X
         if (player.vx > 0 && player.x + player.w < p.x + player.vx + 5) {
            player.x = p.x - player.w;
         } else if (player.vx < 0 && player.x > p.x + p.w - 5) {
            player.x = p.x + p.w;
         }
      }
    }
  ...
  // 4. Input Controls
  useEffect(() => {
    const handleDown = (e: KeyboardEvent) => { gameRef.current.keys[e.key] = true; };
    const handleUp = (e: KeyboardEvent) => { gameRef.current.keys[e.key] = false; };
      ...
      <main className="flex-1 p-6 bg-[#111] text-white">
        ...
        <div className="grid grid-cols-2 gap-4">
          {artists.map(a => (
            <button
              key={a.nome}
              onClick={() => { setSelectedArtist(a); setGameState(GameState.WORLD_MAP); }}
              className="p-4 rounded-2xl bg-card border border-border text-center group active:scale-95"
            >
              <img src={driveImg(a.foto, 300)} className="size-24 rounded-full mx-auto mb-2 border-2 border-primary" alt="" />
  ...
  return (
    <main className="flex-1 flex flex-col bg-[#050505] text-white overflow-hidden">
      {/* UI TopBar */}
      <div className="h-14 border-bottom border-white/10 flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-4">
           <div className="flex flex-col">
             <span className="font-arcade text-[8px] text-muted-foreground mb-1 uppercase">Score</span>
             <span className="font-arcade text-[10px]">{score.toString().padStart(6, "0")}</span>
           </div>
           <div className="flex flex-col">
             <span className="font-arcade text-[8px] text-muted-foreground mb-1 uppercase">Coins</span>
        ...
        <div className="flex gap-2">
           {[...Array(3)].map((_, i) => (
             <Heart key={i} className={`size-4 ${i < lives ? "text-red-500 fill-red-500" : "text-white/20"}`} />
           ))}
        </div>
      </div>
      ...
      <div className="flex-1 relative bg-black grid place-items-center p-2">
         ...
         <AnimatePresence>
            {gameState === GameState.WORLD_MAP && (
               ...
               >
                  <div className="p-8 max-w-lg mx-auto">
                    <h2 className="font-arcade text-xl mb-12 text-center tracking-tighter italic">World Map</h2>
                    <div className="space-y-4">
                       {WORLDS.map((w, i) => (
                          <div key={w.id} className="relative">
                             ...
                             >
                                <div className="flex items-center gap-4">
                                   <div className="size-10 rounded-xl grid place-items-center font-arcade border" style={{ borderColor: w.color, color: w.color }}>{w.id}</div>
                                   <div>
                                      <p className="font-arcade text-[10px] text-white uppercase">{w.name}</p>
                                      <p className="text-[10px] text-muted-foreground mt-1 tracking-widest uppercase">Chart Target: #{100 - (w.id-1)*25}</p>
                                   </div>
                                </div>
                                <div className="flex gap-1 group-hover:translate-x-1 transition-transform">
                                   <Star className={`size-4 ${w.id < currentWorld ? "text-yellow-500 fill-yellow-500" : "text-white/20"}`} />
                                   <Star className={`size-4 ${w.id < currentWorld ? "text-yellow-500 fill-yellow-500" : "text-white/20"}`} />
                                   <Star className={`size-4 ${w.id < currentWorld ? "text-yellow-500 fill-yellow-500" : "text-white/20"}`} />
                                </div>
                             </button>
            ...
            {gameState === GameState.GAME_OVER && (
               ...
               >
                 <div>
                    <h2 className="font-arcade text-4xl mb-4 italic tracking-tighter text-white">WASTED</h2>
                    <p className="font-arcade text-[10px] text-red-300 mb-8 uppercase tracking-widest">Sua carreira estagnou no #{chartPos}</p>
                    <div className="grid grid-cols-2 gap-4">
                       <button 
                         onClick={() => { setLives(3); setGameState(GameState.WORLD_MAP); }}
            ...
            {gameState === GameState.VICTORY && (
               ...
               >
                 <div>
                    ...
                    <h2 className="font-arcade text-4xl mb-4 italic tracking-tighter text-white">CHART TOPPER!</h2>
                    <p className="font-arcade text-[10px] text-yellow-300 mb-8 uppercase tracking-widest">Você alcançou o #{chartPos}!</p>
                    
                    <div className="bg-white/10 p-4 rounded-2xl mb-8 border border-yellow-500/30">
                       <div className="flex justify-between items-center mb-2">
                          <span className="text-[10px] font-arcade uppercase text-muted-foreground">Coins Earned</span>
      ...
      <div className="h-40 bg-[#111] grid grid-cols-2 p-4 shrink-0 border-t border-white/5">
         <div className="grid grid-cols-3 gap-2">
            <ControlBtn icon={<ChevronLeft className="size-8" />} onDown={() => { gameRef.current.keys["ArrowLeft"] = true; }} onUp={() => { gameRef.current.keys["ArrowLeft"] = false; }} />
            <div />
            <ControlBtn icon={<ChevronLeft className="size-8 rotate-180" />} onDown={() => { gameRef.current.keys["ArrowRight"] = true; }} onUp={() => { gameRef.current.keys["ArrowRight"] = false; }} />
         </div>
         <div className="flex items-center justify-end gap-4">
            <ControlBtn 
              color="bg-primary/20 text-primary" 
              icon={<Mic2 className="size-8" />} 
              onDown={() => { gameRef.current.keys["z"] = true; }} 
              onUp={() => { gameRef.current.keys["z"] = false; }} 
            />
            <ControlBtn 
              color="bg-white/20 text-white" 
              icon={<ChevronLeft className="size-8 rotate-90" />} 
              onDown={() => { gameRef.current.keys["ArrowUp"] = true; }} 
              onUp={() => { gameRef.current.keys["ArrowUp"] = false; }} 
            />
         </div>
      </div>
    </main>
  ...
  return (
    <div className="mt-12 pt-8 border-t border-white/10 flex items-center justify-between">
      <div className="flex items-center gap-3">
         <img src={selectedArtist ? driveImg(selectedArtist.foto, 100) : ""} className="size-10 rounded-full border-2 border-primary" alt="" />
         <div>
      ...
      <div className="flex flex-col items-end">
         ...
         <span className="font-arcade text-[8px] uppercase tracking-tighter">Current Hit: "Pop Dreams"</span>
      </div>
    </div>
  );
}
